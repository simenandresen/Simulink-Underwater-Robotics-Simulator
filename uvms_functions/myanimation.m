%-----------------------------------
%
%  animation file
%  all animation called from here
%
%-----------------------------------

%close all;

if (exist('Link')) ==0
    run('./rvctools/startup_rvc.m');
end

%% init robot for animation
global six_link;
for j =1:6
    l(j) = Link([0, DH.d(j), DH.a(j), DH.alpha(j)]);
end
six_link = SerialLink(l,'name','six link');


%% vehicle animation init
rov_vertices_init = [0 -1 0; 0 1 0; -2.3 1 0; -2.3 -1 0;       0 -1 1; 0 1 1; -2.3 1 1; -2.3 -1 1];
my_faces = [1 2 3 4; 2 6 7 3; 4 3 7 8; 1 5 8 4; 1 2 6 5; 5 6 7 8];

%% set figure properties
i=i+1;
h(i)= figure(i);
set(h(i),'Units','normalized');
set(h(i),'Position',[0.1,0.3,0.5,0.4]);
xlabel('x');
ylabel('y');
zlabel('z');
m_i=1;
ws=12;
hold on;

%% animate
ee_traj = [];
step=ceil(length(q_mes(:,1))./100);

view(-37 + 180, 20);
for j=1:step:length(q_mes(:,1))
    g0b=genCordinates2Matrix(xi_mes(1:3,j) , xi_mes(4:6,j));
    new_mat=homtrans(g0b,rov_vertices_init');
    six_link.plot(q_mes(j,:),'noshadow','noname');
    six_link.base=g0b;
    if j>1
        pause(0.1);
        delete(rov_figure);
    end
    rov_figure=patch('Vertices', new_mat', 'Faces', my_faces, 'FaceColor', 'g');
    set(rov_figure,'facealpha',0.5);
    axis([-ws, ws, -ws, ws , -ws, ws]);  
    
    ee_traj(end+1,:)=ee_pose_log(j,:);
    plot3(ee_traj(:,1),ee_traj(:,2),ee_traj(:,3), 'om');
    
end













